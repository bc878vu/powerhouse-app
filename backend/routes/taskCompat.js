...
async function getAssignedUsers(taskId) {
  try {
    ...
    console.warn("TASK COMPAT assignments lookup skipped:", error.message);
    return [];
  }
}
  ...
  try {
    ...
    return (rows || []).map((row) => {
      ...
      const primaryVoice =
        voiceArray[0] ||
        (row.voice_note
          ? { path: row.voice_note, type: "audio/webm" }
          : null);
      ...
      return {
        ...
        assignment_cycle:
          row.assignment_cycle == null
            ? null
            : Number(row.assignment_cycle),
...
async function buildTask(id) {
  ...
  const [assignedUsers, assignmentHistory, completionReports] =
    await Promise.all([
  ...
  const attachments = Array.isArray(parsedFiles)
    ? parsedFiles
    : parsedFiles
      ? [parsedFiles]
      : [];
  ...
  return {
    ...
    assignment_count:
      new Set(
        assignmentHistory.map(
          (item) => Number(item.assignment_cycle || 1)
        )
      ).size || 1,
    ...
    latest_completion:
      completionReports[completionReports.length - 1] || null,
...
async function handleTask(req, res) {
  try {
    ...
    return res.status(500).json({
      success: false,
      msg: error.message || "Failed to load task",
      message: error.message || "Failed to load task",
    });
  }
}

// Report/view and edit-preload use the same complete task payload.
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!userId) {
        userId = cleanId(req.headers["x-user-id"]);
      }

      if (!userId) {
        const assigned = await getAssignedUsers(taskId);
        userId = cleanId(assigned[0]?.user_id);
      }
      ...
      const completionNote = String(
        pick(req.body, ["completion_note", "completionNote", "note"], "") || ""
      ).trim();
      ...
      if (!completionNote && !media.length && !voice.length) {
        ...
        return res.status(400).json({
          success: false,
          msg: "Please add a completion note, media file, or voice note before submitting work",
        });
      }
      ...
      if (!assignment) {
        ...
        return res.status(403).json({
          success: false,
          msg: "This user is not assigned to this task",
        });
      }
      ...
      let assignmentCycle = Number(
        pick(req.body, ["assignment_cycle", "assignmentCycle"], 0)
      ) || 0;
      ...
      if (!assignmentCycle) {
        ...
        assignmentCycle = Number(
          mine[mine.length - 1]?.assignment_cycle ||
      ...
      const insertColumns = Object.keys(payload).filter((key) =>
        completionColumns.has(key)
      );
      ...
      try {
        ...
        if (historyColumns.size) {
          ...
          if (historyColumns.has("completed_at")) historyUpdates.completed_at = new Date();
          if (historyColumns.has("updated_at")) historyUpdates.updated_at = new Date();
        ...
        if (assignmentColumns.has("status")) {
          ...
          if (assignmentColumns.has("completed_at")) updates.completed_at = new Date();
          if (assignmentColumns.has("updated_at")) updates.updated_at = new Date();
          await connection.query(
        ...
        if (taskColumns.size) {
          ...
          if (taskColumns.has("completed_at")) taskUpdates.completed_at = new Date();
          if (taskColumns.has("updated_at")) taskUpdates.updated_at = new Date();
