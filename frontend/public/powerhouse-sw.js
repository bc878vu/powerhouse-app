...
const CACHE_VERSION = "powerhouse-static-v13";
...
function setPushBadge(count = 1) {
  try {
    if (typeof navigator?.setAppBadge === "function") return navigator.setAppBadge(Math.max(1, Number(count) || 1));
  } catch (error) {
...
try {
  ...
  if (firebaseConfig?.apiKey && firebaseConfig?.projectId && firebaseConfig?.appId) {
    ...
    messaging.onBackgroundMessage((payload) => {
      ...
      const notificationId = String(payload.data?.notificationId || payload.messageId || `powerhouse-${Date.now()}`);

      // FCM notification messages are already displayed by the browser/FCM
      // while the PWA is backgrounded. Manually displaying them here would
      // create duplicate notifications. Data-only messages still need our
      // explicit display path.
      const hasDisplayNotification = Boolean(payload.notification?.title || payload.notification?.body);
      if (hasDisplayNotification) {
        void setPushBadge(1);
        return;
      }
...
self.addEventListener("fetch", (event) => {
  ...
  if (url.pathname.startsWith("/assets/") || /\.(?:js|css|woff2?|png|jpg|jpeg|svg|webp|ico)$/.test(url.pathname)) event.respondWith(cacheThenNetwork(request));
});
