...
export default function MyDutyPanel({ compact = false }) {
  ...
  const load = async (silent = false) => {
    ...
    try {
      const [current, monthly] = await Promise.all([API.get(`/duty/user/${userId}`), API.get(`/duty/user/${userId}/monthly`, { params: { year, month } })]);
      setData({ ...(current?.data || {}), ...(monthly?.data || {}) });
    } catch (e) { setError(e?.response?.data?.message || e?.message || "Duty information could not be loaded."); }
  ...
  const recorded = Number(summary.recordedDays || 0), duty = Number(summary.dutyDays || 0), leave = Number(summary.leaveDays || 0), off = Number(summary.offDays || 0);
  const attendance = recorded ? (duty / recorded) * 100 : 0, today = data?.todayDuty, shift = data?.currentShift;
