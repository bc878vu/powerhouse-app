...
async function provisionStaffRecord(firebaseUser) {
  const base = apiBase();
  if (!base || !firebaseUser?.email || isAdminEmail(firebaseUser.email)) return null;
  try {
    const randomPassword = `${globalThis.crypto?.randomUUID?.() || `${Date.now()}-${Math.random()}`}!Aa9`;
    const response = await fetch(`${base}/api/user`, { method: "POST", headers: { "Content-Type": "application/json", Accept: "application/json" }, body: JSON.stringify({ name: firebaseUser.displayName || firebaseUser.email.split("@")[0] || "User", email: firebaseUser.email, password: randomPassword, role: "electrician", status: "active" }) });
...
async function buildSession(firebaseUser) {
  ...
  let profile = null; let profilePermissionIssue = false;
  try {
    const snapshot = await getDoc(profileRef);
    if (snapshot.exists()) profile = { id: snapshot.id, ...snapshot.data() };
    else {
      ...
      try { await setDoc(profileRef, newProfile, { merge: true }); profile = { ...newProfile, id: firebaseUser.uid, role: newProfile.role }; } catch (writeError) { if (writeError?.code === "permission-denied") profilePermissionIssue = true; else throw writeError; }
    }
    const googlePhoto = firebaseUser.photoURL || "";
    if (googlePhoto && (!profile?.profile_pic || profile.profile_pic !== googlePhoto)) {
      try { await setDoc(profileRef, { profile_pic: googlePhoto, profilePic: googlePhoto, photoURL: googlePhoto, updatedAt: serverTimestamp() }, { merge: true }); profile = { ...profile, profile_pic: googlePhoto, profilePic: googlePhoto, photoURL: googlePhoto }; } catch (writeError) { if (writeError?.code === "permission-denied") profilePermissionIssue = true; else console.warn("Could not persist Google profile photo:", writeError?.message || writeError); }
    }
...
export async function loginWithFirebase(email, password) { const normalizedEmail = String(email || "").trim().toLowerCase(); const credential = await signInWithEmailAndPassword(auth, normalizedEmail, password); return buildSession(credential.user); }
export async function loginWithGoogle() { const provider = new GoogleAuthProvider(); provider.setCustomParameters({ prompt: "select_account" }); const credential = await signInWithPopup(auth, provider); return buildSession(credential.user); }
export { signOut };