...
const unwrapUsers = (value) => { if (Array.isArray(value)) return value; if (Array.isArray(value?.users)) return value.users; if (Array.isArray(value?.data)) return value.data; if (Array.isArray(value?.data?.users)) return value.data.users; if (Array.isArray(value?.result)) return value.result; return []; };
const profileValue = (value) => { if (!value) return ""; if (typeof value === "object") return String(value.url || value.downloadURL || value.path || ""); return String(value).trim(); };

const isPlaceholderUser = (user) => {
  const name = String(user?.name || user?.displayName || "").trim().toLowerCase();
  const email = String(user?.email || "").trim().toLowerCase();
  if (!name && !email) return true;
  if (new Set(["vcvf", "lklkj", "jhhk", "drgdfgfd fsgd"]).has(name)) return true;
  if (/^(dummy|test|testing|sample|demo|fake|temp|temporary|asdf|qwerty|user\s*\d*|new\s*user)$/i.test(name)) return true;
...
const readStoredUsers = () => { try { if (typeof window === "undefined") return []; const parsed = JSON.parse(window.localStorage.getItem(USER_STORAGE_KEY) || "[]"); return Array.isArray(parsed) ? parsed.map(normalizeUser).filter((user) => !isPlaceholderUser(user) && (user.id || user.uid)) : []; } catch { return []; } };
...
const saveUsers = (users) => { const normalized = users.map(normalizeUser).filter((user) => !isPlaceholderUser(user) && (user.id || user.uid)); userCache.data = normalized; userCache.at = Date.now(); writeStoredUsers(normalized); return normalized; };

const ensureNumericIds = async (users) => {
  const cleanUsers = users.map(normalizeUser).filter((user) => !isPlaceholderUser(user));
  let nextId = 10000;
  try {
    const snapshot = await getDocs(collection(db, "powerhouse_users"));
    const numericIds = snapshot.docs.map((item) => Number(item.data()?.numericId ?? item.data()?.id)).filter((value) => Number.isInteger(value) && value >= 10000);
    if (numericIds.length) nextId = Math.max(...numericIds) + 1;
  } catch {}
  for (const user of cleanUsers) {
    if (Number.isInteger(Number(user.numericId)) && Number(user.numericId) > 0) continue;
    const numericId = Number.isInteger(Number(user.id)) && Number(user.id) > 0 ? Number(user.id) : nextId++;
    user.id = String(numericId);
    user.numericId = numericId;
    const uid = String(user.uid || user.firebaseUid || "").trim();
    if (uid) {
      try { await setDoc(doc(db, "powerhouse_users", uid), { id: numericId, numericId, uid, firebaseUid: uid, updatedAt: serverTimestamp() }, { merge: true }); } catch (error) { console.warn("Numeric user ID persistence skipped:", error?.message || error); }
    }
  }
  return cleanUsers;
};
...
const mergeUsers = (legacyUsers, firebaseUsers) => {
  ...
  const add = (raw, source) => {
    ...
    const merged = existing ? { ...existing, ...user, ...(source === "firebase" ? user : {}), id: Number.isInteger(Number(existing.id)) ? existing.id : user.id } : { ...user };
  ...
  return [...new Set([...byEmail.values(), ...byUid.values()])].filter((user) => !isPlaceholderUser(user)).map(normalizeUser).sort((a, b) => Number(b.id || 0) - Number(a.id || 0) || String(a.name || "").localeCompare(String(b.name || "")));
};

const withTimeout = (promise, ms, message = "Request timed out") => { let timer; const timeout = new Promise((_, reject) => { timer = setTimeout(() => reject(new Error(message)), ms); }); return Promise.race([promise, timeout]).finally(() => clearTimeout(timer)); };
const legacyGet = async (path) => { const base = getLegacyBase(); if (!base) throw new Error("VITE_API_URL is not configured"); const controller = typeof AbortController !== "undefined" ? new AbortController() : null; const timer = controller ? setTimeout(() => controller.abort(), LEGACY_TIMEOUT) : null; try { const response = await fetch(`${base}/api${path}`, { headers: { Accept: "application/json" }, credentials: "include", signal: controller?.signal }); if (!response.ok) throw new Error(`Legacy API ${response.status}`); return response.json(); } finally { if (timer) clearTimeout(timer); } };
...
const refreshUsers = async () => {
  ...
  userCache.promise = (async () => {
    ...
    if (!firebaseUsers.length) { try { firebaseUsers = unwrapUsers(await withTimeout(requestFirebase("GET", "/user/all"), REQUEST_TIMEOUT, "Firebase user list route timed out")); } catch (e) { console.warn("Firebase user list route unavailable:", e?.message || e); } }
    firebaseUsers = await ensureNumericIds(firebaseUsers);
...
const extractAssignedIds = (data, result) => { if (typeof FormData !== "undefined" && data instanceof FormData) { const values = data.getAll("user_ids[]").map(String).filter(Boolean); if (values.length) return [...new Set(values)]; const single = String(data.get("user_id") || "").trim(); if (single) return [single]; } const raw = result?.assigned_user_ids || result?.user_ids || (result?.user_id ? [result.user_id] : []); return [...new Set((Array.isArray(raw) ? raw : [raw]).map((item) => String(item?.id ?? item?.uid ?? item?.user_id ?? item).trim()).filter(Boolean))]; };
const notifyTaskAssignees = async (result, data) => { try { const selectedIds = extractAssignedIds(data, result); if (!selectedIds.length) return; const users = await getUsersFast(); const recipients = users.filter((user) => selectedIds.some((id) => String(user.id) === String(id) || String(user.uid) === String(id) || String(user.firebaseUid) === String(id)) && String(user.status || "active").toLowerCase() !== "inactive"); const recipientUids = [...new Set(recipients.map((user) => String(user.uid || user.firebaseUid || "").trim()).filter(Boolean))]; if (!recipientUids.length) return; const title = String(result?.title || data?.get?.("title") || "New Task Assigned").trim(); const priority = String(result?.priority || data?.get?.("priority") || "High").trim(); const taskId = String(result?.id || result?.document_id || result?.task_id || "").trim(); const route = taskId ? `/task-view/${taskId}` : "/notifications"; const notificationId = `task-assigned-${taskId || Date.now()}-${recipientUids.join("-")}`; await Promise.all(recipientUids.map((uid) => createNotification(uid, { title: "📋 Task Assigned To You", body: `${title} (${priority} priority)`, type: "task_assigned", route, taskId, sourceId: taskId || null }).catch((error) => console.warn("TASK IN-APP NOTIFICATION ERROR:", error?.message || error)))); await sendPushNotification({ title: "📋 Task Assigned To You", body: `${title} (${priority} priority)`, route, userIds: recipientUids, notificationId }); } catch (error) { console.warn("TASK PUSH DELIVERY FAILED:", error?.message || error); } };
...
const API = {
  get: async (url, config = {}) => {
    ...
    if (cleanUrl.startsWith("/task/my-tasks/") || cleanUrl.startsWith("task/my-tasks/")) { try { const result = await firebaseRequest("GET", url, undefined, config?.params || {}); const rawTasks = Array.isArray(result) ? result : (Array.isArray(result?.tasks) ? result.tasks : []); const enriched = await enrichTaskProfiles(rawTasks); const normalized = enriched.map(normalizeTaskForDisplay); normalized.forEach((task) => { if (task?.document_id && task?.task_number) taskRouteMap.set(String(Number(task.task_number)), String(task.document_id)); }); return { data: normalized, tasks: normalized }; } catch (firebaseError) { const legacyPath = cleanUrl.startsWith("/") ? cleanUrl : `/${cleanUrl}`; try { return { data: await legacyGet(legacyPath) }; } catch { throw firebaseError; } } }
    try {
      const result = await firebaseRequest("GET", url, undefined, config?.params || {});
      if (cleanUrl.startsWith("/user/") || cleanUrl.startsWith("user/")) { const id = cleanUrl.split("/")[2], users = await getUsersFast(), profile = users.find((user) => String(user.id) === String(id) || String(user.uid) === String(id) || String(user.firebaseUid) === String(id)); if (cleanUrl.startsWith("/user/full/") || cleanUrl.startsWith("user/full/")) return { data: { ...result, user: { ...(profile || {}), ...(result?.user || {}) } } }; return { ...(profile || {}), ...(result || {}) }; }
