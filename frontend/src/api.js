...
import { activityStats as taskActivityStats, assignTask as createTask, completeTask, deleteTask, getTask, isTaskPath, listMyTasks, listUserTasks, updateTask, updateTaskStatus } from "./services/taskService";
...
const makeDisplayIds = (values, assignedUsers = [], fallbackNames = []) => {
  ...
  const names = Array.isArray(assignedUsers) ? assignedUsers.map((user) => String(user?.name || user?.full_name || user?.displayName || user?.email || "").trim()).filter(Boolean) : [];
...
const requestFirebaseApi = async (method, url, data, config = {}) => {
  ...
  if (path.startsWith("user/full/") && method === "GET") {
    return withTimeout(requestFirebase(method, url, normalizeTaskRequest(method, url, data).then(async (result) => {
