...
const normalizeAssignedUsers = (task) => {
  ...
  return { ...task, assigned_users: assignedUsers.length ? assignedUsers : task.assigned_users, assigned_staff_names: staffNames, assigned_staff_emails: staffEmails, assigned_staff_roles: staffRoles, assigned_user_ids: Array.isArray(task.assigned_user_ids) ? task.assigned_user_ids.map(String) : task.assigned_user_ids };
};
...
const sortTasks = (tasks) => [...(Array.isArray(tasks) ? tasks : [])].sort((a, b) => {
  const sa = statusRank[normStatus(a?.status)] ?? 1, sb = statusRank[normStatus(b?.status)] ?? 1;
  if (sa !== sb) return sa - sb;
  if (sa === 0) { const pa = priorityRank[String(a?.priority || "Medium").toLowerCase()] ?? 2, pb = priorityRank[String(b?.priority || "Medium").toLowerCase()] ?? 2; if (pa !== pb) return pa - pb; }
  const ta = new Date(a?.assigned_at || a?.created_at || a?.updated_at || 0).getTime() || 0, tb = new Date(b?.assigned_at || b?.created_at || b?.updated_at || 0).getTime() || 0;
...
const requestTaskApi = async (method, url, data, config = {}) => { const path = taskPath(url); return isTaskPath(path) ? requestTaskHttp(method, path, normalizeTaskRequest(method, url, data), config) : null; };
...
const request = async (method, url, data, config = {}) => {
  try {
    const path = taskPath(url);
    // Production has no guaranteed SQL API deployment. Stats therefore use the same
    // Firebase source as the operational dashboard unless SQL stats are explicitly enabled.
    const result = path === "activity/stats" && method === "GET"
      ? (isSqlStatsEnabled() ? await requestTaskHttp("GET", "activity/stats", undefined, config) : await withTimeout(requestFirebase("GET", url, undefined, config?.params || {}), config?.timeout || 20000, "Firebase dashboard stats timed out"))
