...
import { Activity, ArrowLeft, BarChart3, CalendarClock, Cpu, Edit3, Gauge, Image as ImageIcon, Loader2, Wrench, Zap } from "lucide-react";
...
export default function MachineDetails() {
  ...
  const [roleLabel, roleClass] = ROLE[machine.utilityRole] || ROLE.consumer;

  return <div className="space-y-6 animate-in fade-in duration-500">
    <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4"><div className="flex items-center gap-3"><button onClick={() => navigate("/machines")} className="w-11 h-11 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center"><ArrowLeft size={19} /></button><div><p className="text-[9px] uppercase tracking-widest text-yellow-500 font-black">Machine Information</p><h1 className="text-2xl md:text-3xl font-black mt-1">{machine.name || "Unnamed Machine"}</h1><p className="text-slate-500 text-sm mt-1">{machine.code || "No code"} · {machine.location || "No area assigned"}</p></div></div><div className="flex flex-wrap gap-2 items-center"><Badge className={statusClass}>{statusLabel}</Badge>{isAdmin && <button onClick={() => navigate(`/machines/edit/${machine.id}`)} className="px-4 py-3 rounded-xl bg-yellow-500 text-black text-xs font-black"><Edit3 size={15} className="inline mr-1" /> EDIT MACHINE</button>}</div></div>

    <section className="grid grid-cols-1 lg:grid-cols-[minmax(0,1.05fr)_minmax(320px,.95fr)] gap-5"><div className="rounded-[2rem] border border-white/5 bg-[#020617] overflow-hidden min-h-[320px] flex items-center justify-center"><div className="relative w-full h-full min-h-[320px] flex items-center justify-center">{machine.imageUrl ? <img src={machine.imageUrl} alt={machine.name || "Machine"} className="w-full h-full max-h-[520px] object-contain bg-black/20" onError={event => { event.currentTarget.style.display = "none"; event.currentTarget.parentElement.querySelector(".machine-image-fallback")?.classList.remove("hidden"); }} /> : null}<div className={`machine-image-fallback ${machine.imageUrl ? "hidden" : ""} text-center text-slate-700 p-10`}><ImageIcon size={56} className="mx-auto" /><p className="mt-3 text-xs font-black uppercase tracking-widest">No machine image available</p>{machine.imageUrl && <p className="text-[10px] text-slate-600 mt-2">The supplied image URL could not be loaded.</p>}</div></div></div><div className="rounded-[2rem] border border-white/5 bg-[#020617] p-6"><div className="flex items-center gap-3"><div className="w-11 h-11 rounded-2xl bg-yellow-500 text-black flex items-center justify-center"><Cpu size={21} /></div><div><p className="text-[9px] uppercase tracking-widest text-slate-600 font-black">Machine Overview</p><h2 className="font-black text-lg">{machine.manufacturer || "Manufacturer not specified"}</h2></div></div><div className="grid grid-cols-2 gap-3 mt-6"><Field label="Category" value={machine.category} /><Field label="Type" value={machine.type} /><Field label="Model" value={machine.model} /><Field label="Serial Number" value={machine.serialNumber} /><Field label="Department" value={machine.department} /><Field label="Area / Location" value={machine.location} /></div></div></section>

    <section className="rounded-[2rem] border border-yellow-500/10 bg-[#020617] p-6"><div className="flex flex-wrap items-center gap-3"><div><p className="text-[9px] uppercase tracking-widest text-slate-600 font-black">Utility Classification</p><h2 className="font-black mt-1">{machine.utilityType || "Electricity"}</h2></div><Badge className={roleClass}>{roleLabel}</Badge></div><p className="text-sm text-slate-400 mt-4">This machine is classified as <strong className="text-white">{roleLabel.toLowerCase()}</strong> for <strong className="text-yellow-400">{machine.utilityType || "Electricity"}</strong>. This classification is stored with the machine record and remains visible in its details and register.</p></section>
    ...
    <section className="grid grid-cols-2 lg:grid-cols-5 gap-3"><Field label="Rated Capacity" value={`${n(machine.capacity).toLocaleString()} ${machine.capacityUnit || "kW"}`} /><Field label="Current Running Load" value={`${n(machine.currentRunningLoad).toLocaleString()} ${machine.loadUnit || "kW"}`} /><Field label="Live Utilization" value={`${stats.utilization.toFixed(1)}%`} /><Field label="Normal Load Factor" value={`${n(machine.normalLoadFactor).toFixed(1)}%`} /><Field label="Status" value={statusLabel} /></section>
    ...
    <section className="rounded-[2rem] border border-white/5 bg-[#020617] p-6"><div className="flex items-center justify-between gap-3"><div><h2 className="font-black">Activity & Load History</h2><p className="text-xs text-slate-500 mt-1">Every saved operational load record for this machine is retained here as its activity history.</p></div><span className="rounded-full bg-white/5 px-3 py-2 text-[9px] font-black text-slate-400">{logs.length} RECORDS</span></div><div className="overflow-x-auto mt-5"><table className="w-full min-w-[900px]"><thead><tr className="border-b border-white/5">{["Date","Activity","Actual Load","Peak","Hours","Energy","Status","Recorded By","Note"].map(label => <th key={label} className="px-3 py-3 text-left text-[9px] uppercase tracking-widest text-slate-500">{label}</th>)}</tr></thead><tbody>{logs.map(log => <tr key={log.id} className="border-b border-white/[0.04]"><td className="px-3 py-3 text-xs">{log.date || "—"}</td><td className="px-3 py-3 text-xs font-black text-blue-300">{log.activityType === "load_record" ? "Load Record" : "Machine Activity"}</td><td className="px-3 py-3 text-xs font-bold text-yellow-300">{n(log.actualLoad)} {machine.loadUnit || "kW"}</td><td className="px-3 py-3 text-xs text-orange-300">{n(log.peakLoad || log.actualLoad).toFixed(1)}</td><td className="px-3 py-3 text-xs">{n(log.operatingHours).toFixed(2)}</td><td className="px-3 py-3 text-xs font-black text-yellow-400">{energy(log).toFixed(2)} kWh</td><td className="px-3 py-3 text-xs">{STATUS[log.status]?.[0] || log.status || "—"}</td><td className="px-3 py-3 text-xs text-slate-400">{log.recordedBy || "—"}</td><td className="px-3 py-3 text-xs text-slate-500 max-w-xs">{log.note || "—"}</td></tr>)}</tbody></table>{logs.length === 0 && <div className="py-12 text-center text-slate-600 text-xs font-black uppercase tracking-widest">No activity/load history recorded yet</div>}</div></section>
  </div>;
}

function Metric({ title, value, unit, icon: Icon }) { return <div className="rounded-2xl border border-white/5 bg-white/[0.025] p-4"><div className="flex items-center justify-between"><p className="text-[9px] uppercase tracking-widest text-slate-500 font-black">{title}</p><Icon size={16} className="text-yellow-400" /></div><p className="mt-2 text-xl md:text-2xl font-black">{value}<span className="ml-1 text-xs text-slate-500">{unit || ""}</span></p></div>; }
