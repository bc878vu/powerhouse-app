...
export async function listPanels({ includeDeleted = false } = {}) {
  ...
  try {
    ...
    // Firestore may require a composite index for the filtered query.
...
export async function createPanel(payload) {
  ...
  await addDoc(collection(db, "powerhouse_panels"), panel);

  // Move the generated numeric id into the document key so all existing UI
  // routes can continue to use /add-panel/:id without another mapping table.
...
export function subscribeToPanels(callback) {
  return onSnapshot(panelsRef, (snapshot) => {
    const panels = snapshot.docs
      .map(fromDoc)
      .filter((panel) => panel.is_deleted !== true);
    callback(panels);
  }, (error) => {
    console.error("Firestore panel subscription error:", error);
  });
}
...
export async function panelRequest(method, url, data) {
  ...
  if (method === "GET" && parts[1] === "history" && parts[2] === "deleted" && parts[3]) {
    const panel = await getPanel(parts[3]);
    return { panel, success: Boolean(panel) };
  }
  if (method === "PUT" && parts[1] === "history" && parts[2] === "deleted" && parts[3] && parts[4] === "restore") {
    return { success: true, panel: await restorePanel(parts[3]), message: "Panel restored successfully." };
  }
  if (method === "DELETE" && parts[1] === "history" && parts[2] === "deleted" && parts[3] && parts[4] === "permanent") {
    return permanentlyDeletePanel(parts[3]);
  }
  if (method === "GET" && parts[1] && parts[2] === "stats") return getPanelStats(parts[1]).then((stats) => stats);
  if (method === "GET" && parts[1]) {
    const panel = await getPanel(parts[1]);
    if (!panel) throw new Error("Panel not found");
    return { panel };
  }
  if (method === "PUT" && parts[1] && parts[2] === "status") {
    return { success: true, panel: await updatePanelStatus(parts[1], data?.status, data?.reason) };
  }
