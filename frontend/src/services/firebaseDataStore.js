...
async function listDuties() {
  ...
  dutiesPromise = allDocs(dutiesRef).then((items) => {
    dutiesCache = items;
...
async function listTools() {
  ...
  toolsPromise = allDocs(toolsRef).then((items) => {
    toolsCache = items;
...
async function uploadFile(file, folder) {
  ...
  return {
    ...
    name: file.name || safeName, type: file.type || "application/octet-stream", size: file.size || 0
  };
}
...
export async function listUsers() {
  ...
  usersPromise = allDocs(usersRef)
    ...
    .then((items) => { usersCache = items; usersCacheAt = Date.now(); return items; })
...
export async function createUser(payload) {
  ...
  await setDoc(doc(db, "powerhouse_users", uid), {
    ...profile, uid, id: profile.id || uid, email: email.trim(), status: profile.status || "active",
...
export async function updateUser(id, payload) {
  ...
  const next = { ...payload };
  delete next.password; delete next.id;
...
export async function deleteUser(id) {
  ...
  invalidateUsers(); invalidateDuties();
  return { success: true };
}
...
export async function listTasks() {
  ...
  tasksPromise = allDocs(tasksRef)
    ...
    .then((items) => { tasksCache = items; tasksCacheAt = Date.now(); return items; })
    .finally(() => { tasksPromise = null; });
  return tasksPromise;
}

// Resolve a real Firestore ID, a stored numeric ID, or the legacy 01/02/03 display ID.
...
export async function assignTask(payload) {
  ...
  const numericTaskId = String(nextNumber).padStart(2, "0");
  const history = ids.map((userId) => ({ user_id: userId, assignment_cycle: 1, status: payload.status || "Pending", assigned_at: created }));
  const task = {
    ...payload, task_number: numericTaskId, display_id: numericTaskId,
    assigned_user_ids: ids, user_ids: ids, user_id: ids[0] || payload.user_id || "",
    status: payload.status || "Pending", assignment_cycle: 1, assignment_count: 1,
    assignment_history: history, created_at: created, updated_at: created, createdAt: serverTimestamp()
  };
  const docRef = await addDoc(tasksRef, task);
  await addDoc(activitiesRef, { type: "task_created", task_id: docRef.id, status: task.status, task_number: numericTaskId, created_at: serverTimestamp() });
  invalidateTasks();
...
async function updateTask(id, payload) {
  ...
  const next = { ...payload };
  delete next.id; delete next.createdAt; delete next.task_number; delete next.display_id;
  const updatedAt = nowIso();
  await updateDoc(target.ref, { ...next, updated_at: updatedAt });
  invalidateTasks();
  return { ...existing, ...next, id: target.id, updated_at: updatedAt };
}
...
export async function dutyStaff(year, month) {
  ...
  return users.map((user) => {
    ...
    return { ...user, currentShift: shifts[0] || null, todayDuty, monthlySummary: {
...
export async function dutyHistory(userId) {
  ...
  return {
    shifts: records.filter((item) => item.record_type === "shift").sort((a, b) => String(b.effective_from || "").localeCompare(String(a.effective_from || ""))),
    duties: records.filter((item) => item.record_type === "status").sort((a, b) => String(b.duty_date || "").localeCompare(String(a.duty_date || "")))
  };
}
...
export async function activityStats() {
  ...
  const ordered = [...tasks].sort((a, b) => String(a.created_at || a.createdAt || "").localeCompare(String(b.created_at || b.createdAt || "")));
  const activities = tasks.slice(0, 100).map((task) => {
    const index = ordered.findIndex((item) => String(item.id) === String(task.id));
    const number = String(task.task_number || task.display_id || index + 1).padStart(2, "0");
    return { ...task, task_number: number, display_id: number };
  });
  return {
    staffCount: users.length, taskCount: tasks.length, pendingCount: counts.Pending,
    inProgressCount: counts["In Progress"], completedCount: counts.Completed, rejectedCount: counts.Rejected,
    activities,
    onDutyToday: { count: summary.onDutyToday, staff: [] },
    ...
    operationalSummary: {
      onDutyCount: summary.onDutyToday,
      panelsUnderWorkCount: panels.filter((p) => p.status === "live").length,
      panelsOffCount: panels.filter((p) => p.status === "off").length,
      panelsMaintenanceCount: panels.filter((p) => p.status === "maintenance").length
    },
    serverDate: nowIso()
  };
}
...
export async function requestFirebase(method, url, data, params = {}) {
  ...
  if (path === "tools/assign" && method === "POST") {
    ...
    const record = {
      ...payload, userId, user_id: userId, assigned_to: userId,
      userName: String(payload.userName || ""), toolName, tool_name: toolName,
      category: String(payload.category || "General"), quantity: Math.max(1, Number(payload.quantity) || 1),
      date: payload.date || new Date().toISOString().slice(0, 10), status: payload.status || "assigned",
      assigned_at: nowIso(), created_at: nowIso(), assigned_by: auth.currentUser?.uid || ""
    };
  ...
  if (path === "categories" && method === "POST") {
    const r = await addDoc(categoriesRef, payload);
    return { id: r.id, ...payload };
  }
