...
export async function addMachine(machine) {
  ...
  if (!data.name || !data.code) throw new Error("Machine name and code are required.");
  if (data.capacity > 0 && data.currentRunningLoad > data.capacity) throw new Error("Actual running load cannot exceed rated load.");
  return addDoc(collection(db, MACHINE_COLLECTION), { ...data, createdAt: serverTimestamp(), updatedAt: serverTimestamp() });
}
...
export async function updateMachine(id, machine) {
  ...
  if (!data.name || !data.code) throw new Error("Machine name and code are required.");
  if (data.capacity > 0 && data.currentRunningLoad > data.capacity) throw new Error("Actual running load cannot exceed rated load.");
  await updateDoc(doc(db, MACHINE_COLLECTION, id), { ...data, updatedAt: serverTimestamp() });
}
...
export async function addMachineLoadLog(log) {
  ...
  const actualLoad = cleanNumber(log.actualLoad);
  const operatingHours = cleanNumber(log.operatingHours);
  const peakLoad = Math.max(actualLoad, cleanNumber(log.peakLoad));
  const meterStart = log.meterStart === "" ? null : Number(log.meterStart);
  const meterEnd = log.meterEnd === "" ? null : Number(log.meterEnd);
  const meterEnergy = Number.isFinite(meterStart) && Number.isFinite(meterEnd) && meterEnd >= meterStart ? meterEnd - meterStart : 0;
  const directEnergy = cleanNumber(log.energyConsumed);
  const energyConsumed = Number((meterEnergy || directEnergy || actualLoad * operatingHours).toFixed(2));
  ...
  batch.set(logRef, {
    ...
    activityType: "load_record",
    date,
    actualLoad,
    peakLoad,
    operatingHours,
    meterStart: Number.isFinite(meterStart) ? meterStart : null,
    meterEnd: Number.isFinite(meterEnd) ? meterEnd : null,
    energyConsumed,
    status,
    note: String(log.note || "").trim(),
    recordedBy: String(log.recordedBy || "").trim(),
...
export async function deleteMachineLoadLog(id) {
  if (!id) throw new Error("Load record ID is required.");
...
export async function addMachineCategory(name) {
  ...
  return addDoc(collection(db, CATEGORY_COLLECTION), { name: clean, createdAt: serverTimestamp(), updatedAt: serverTimestamp() });
}
