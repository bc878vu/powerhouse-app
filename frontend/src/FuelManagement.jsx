...
const ENGINES = Object.fromEntries(Object.entries(GENERATOR_MASTER_DATA).map(([key, cfg]) => [key, { label: cfg.label, ratedKVA: cfg.ratedKVA, ratedKW: cfg.ratedKW }]));
...
export default function FuelManagement() {
  ...
  const previousMeter = (engine, date) => {
    const base = GENERATOR_METER_BASELINES[engine];
    ...
    const lastCurrent = last && last.currentHours !== "" && last.currentHours != null ? hours(last.currentHours) : 0;
    return Math.max(base?.runningHours || 0, lastCurrent, rows.reduce((s,x) => s + num(x.duration), 0));
  };
  ...
  const stats = useMemo(() => Object.entries(ENGINES).map(([key,cfg]) => {
    ...
    const baseline = GENERATOR_METER_BASELINES[key] || {};
    const futureRows = rows.filter(x => String(x.date || "") >= String(baseline.capturedDate || today()));
    const latestCurrent = rows.reduce((max,x) => Math.max(max, num(x.currentHours)), 0);
    const total = Math.max(num(baseline.runningHours), latestCurrent, num(baseline.runningHours) + futureRows.reduce((s,x) => s + num(x.duration), 0));
    const futureKwh = futureRows.reduce((s,x) => s + num(x.kwh),0);
    const kwh = Math.max(num(baseline.kwh), num(baseline.kwh) + futureKwh);
    ...
    const since = service ? Math.max(0,total-num(service.engineHoursAtService)) : 0;
