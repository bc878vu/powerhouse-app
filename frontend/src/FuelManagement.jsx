...
export default function FuelManagement() {
  ...
  useEffect(() => {
    ...
    return () => { stopEntries(); stopServices(); };
  }, []);
  ...
  return <div className="space-y-5">
    <div className="rounded-[2rem] border border-white/5 bg-[#020617] overflow-hidden">
      <div className="p-4 md:p-5 border-b border-white/5 flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-3"><div className="w-11 h-11 rounded-2xl bg-yellow-500 text-black flex items-center justify-center"><Fuel size={22}/></div><div><h1 className="text-xl md:text-2xl font-black">Fuel Management</h1><p className="text-[9px] text-slate-500 uppercase tracking-[.2em]">Diesel • load calculation • generator maintenance</p></div></div><div className="flex flex-wrap gap-2">{[["dashboard","Dashboard",LayoutDashboard],["entry","New Entry",Plus],["reports","Reports",FileText]].map(([id,text,Icon]) => <button key={id} onClick={() => setView(id)} className={view === id ? "px-4 py-2.5 rounded-xl text-xs font-black bg-yellow-500 text-black flex items-center gap-2" : "px-4 py-2.5 rounded-xl text-xs font-black bg-white/5 text-slate-300 flex items-center gap-2"}><Icon size={15}/>{text}</button>)}<button onClick={() => window.location.reload()} className="p-2.5 rounded-xl bg-white/5"><RefreshCw size={15} className={loading ? "animate-spin" : ""}/></button></div></div>
      ...
      {view === "dashboard" && <div className="p-4 md:p-7 space-y-6"><div className="flex justify-between"><div><h2 className="text-2xl font-black">Fuel Dashboard</h2><p className="text-slate-500 text-sm mt-1">Live generator runtime, fuel consumption and service alerts.</p></div><span className="text-[9px] uppercase tracking-widest text-green-400 font-black">{loading ? "SYNCING" : "FIRESTORE LIVE"}</span></div>
