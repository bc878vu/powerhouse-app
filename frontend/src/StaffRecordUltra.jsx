...
import { ChevronLeft, ChevronRight, Eye, Search, Users, UserCheck, UserX, Palmtree, Phone, Mail, MapPin } from "lucide-react";
...
export default function StaffRecordUltra() {
  ...
  const roles = useMemo(
    () => [...new Set(users.map((u) => normalize(u.role || u.category).toLowerCase()).filter(Boolean))],
    [users]
  );
  ...
  const filtered = useMemo(() => {
    ...
    return users.filter((u) => {
      const text = [u.name, u.email, u.phone, u.employeeID, u.id, u.role, u.category, u.address]
  ...
  return (
    <div className="min-h-screen bg-[#080d1d] px-4 py-7 text-white md:px-8">
      <div className="mx-auto max-w-[1650px]">
        <div className="mb-7">
          <p className="mb-2 text-xs font-bold uppercase tracking-[0.28em] text-yellow-400">Workforce Management</p>
          <h1 className="text-3xl font-black md:text-4xl">Staff Records</h1>
          <p className="mt-2 text-sm text-slate-400">Fast employee directory with complete available staff information.</p>
        </div>
        ...
        <div className="mb-5 grid gap-3 rounded-2xl border border-white/10 bg-[#11182c] p-4 lg:grid-cols-[1fr_190px_190px]">
          <label className="flex items-center gap-3 rounded-xl bg-[#1c2740] px-4">
            ...
            <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search ID, name, email, phone, address..." className="w-full bg-transparent py-3 text-sm outline-none placeholder:text-slate-500" />
          </label>
          ...
          <select value={status} onChange={(e) => setStatus(e.target.value)} className="rounded-xl bg-[#1c2740] px-4 py-3 text-sm outline-none">
        ...
        <div className="overflow-hidden rounded-2xl border border-white/10 bg-[#0e1528] shadow-2xl">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/10 px-5 py-4">
            <div>
              <h2 className="font-bold">Employee Directory</h2>
              <p className="mt-1 text-xs text-slate-500">{filtered.length ? `${pageStart + 1}-${Math.min(pageStart + PAGE_SIZE, filtered.length)}` : 0} of {filtered.length} matching staff members · {users.length} total</p>
            </div>
            {!loading && pageCount > 1 && (
              <div className="flex items-center gap-2">
                <button type="button" disabled={safePage <= 1} onClick={() => setPage((p) => Math.max(1, p - 1))} className="rounded-lg border border-white/10 bg-white/[0.03] p-2 text-slate-300 disabled:cursor-not-allowed disabled:opacity-30" aria-label="Previous page"><ChevronLeft size={17} /></button>
                <span className="min-w-[100px] text-center text-xs font-semibold text-slate-400">Page {safePage} / {pageCount}</span>
                <button type="button" disabled={safePage >= pageCount} onClick={() => setPage((p) => Math.min(pageCount, p + 1))} className="rounded-lg border border-white/10 bg-white/[0.03] p-2 text-slate-300 disabled:cursor-not-allowed disabled:opacity-30" aria-label="Next page"><ChevronRight size={17} /></button>
              </div>
            )}
          </div>
          ...
          {loading ? (
            <div className="flex min-h-[360px] items-center justify-center text-slate-400">
              <div className="text-center"><div className="mx-auto mb-3 h-7 w-7 animate-spin rounded-full border-2 border-yellow-500/30 border-t-yellow-500" /><p>Loading staff records…</p></div>
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[1250px] text-left text-sm">
                <thead className="bg-[#151f35] text-xs uppercase tracking-wider text-slate-400">
                  <tr><th className="px-5 py-4">ID</th><th className="px-5 py-4">Staff</th><th className="px-5 py-4">Contact</th><th className="px-5 py-4">Role / Status</th><th className="px-5 py-4">Personal Information</th><th className="px-5 py-4">Address</th><th className="px-5 py-4">Actions</th></tr>
                </thead>
                <tbody className="divide-y divide-white/5">
                  {visibleUsers.map((u, localIndex) => {
                    const globalIndex = pageStart + localIndex;
                    const s = normalize(u.status || "active").toLowerCase();
                    return <tr key={u.id || u.uid || `${u.email}-${globalIndex}`} className="hover:bg-white/[0.025]">
                      <td className="px-5 py-5"><span className="rounded-lg bg-yellow-500/10 px-3 py-2 font-bold text-yellow-400">{displayId(u, globalIndex)}</span></td>
                      ...
                      <td className="px-5 py-5"><button type="button" onClick={() => navigate(`/user/${u.id || u.uid}`)} className="inline-flex items-center gap-2 rounded-xl bg-blue-500/15 px-4 py-2.5 font-semibold text-blue-300 hover:bg-blue-500/25"><Eye size={16} /> View Details</button></td>
                    </tr>;
                  })}
                </tbody>
              </table>
...
function Avatar({ user }) {
  const name = normalize(user.name || user.email || "U");
  const initials = name.split(/\s+/).filter(Boolean).slice(0, 2).map((x) => x[0]).join("").toUpperCase() || "U";
  const src = normalize(user.profile_pic);
  return <div className="flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-full border border-yellow-500/50 bg-yellow-500/10 font-bold text-yellow-400">
    {src ? <img src={src} alt="" loading="lazy" decoding="async" className="h-full w-full object-cover" /> : initials}
  </div>;
}

function Stat({ icon, value, label }) {
  return <div className="rounded-2xl border border-white/10 bg-[#10182c] p-5"><div className="mb-4 inline-flex rounded-xl bg-yellow-500/10 p-3 text-yellow-400">{icon}</div><div className="text-3xl font-black">{value}</div><div className="mt-1 text-xs font-semibold uppercase tracking-wider text-slate-500">{label}</div></div>;
}
