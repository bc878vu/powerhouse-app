...
const displayId = (_user, index) => `#${String(index + 1).padStart(3, "0")}`;
    ...
    <div className="min-h-screen bg-[#080d1d] px-4 py-7 text-white md:px-8">
      <div className="mx-auto max-w-[1650px]">
        ...
        <div className="overflow-hidden rounded-2xl border border-white/10 bg-[#0e1528] shadow-2xl">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/10 px-5 py-4"><div><h2 className="font-bold">Employee Directory</h2><p className="mt-1 text-xs text-slate-500">{filtered.length ? `${pageStart + 1}-${Math.min(pageStart + PAGE_SIZE, filtered.length)}` : 0} of {filtered.length} matching staff members · {users.length} total</p></div>{!loading && pageCount > 1 && <div className="flex items-center gap-2"><button type="button" disabled={safePage <= 1} onClick={() => setPage((p) => Math.max(1, p - 1))} className="rounded-lg border border-white/10 bg-white/[0.03] p-2 text-slate-300 disabled:cursor-not-allowed disabled:opacity-30" aria-label="Previous page"><ChevronLeft size={17} /></button><span className="min-w-[100px] text-center text-xs font-semibold text-slate-400">Page {safePage} / {pageCount}</span><button type="button" disabled={safePage >= pageCount} onClick={() => setPage((p) => Math.min(pageCount, p + 1))} className="rounded-lg border border-white/10 bg-white/[0.03] p-2 text-slate-300 disabled:cursor-not-allowed disabled:opacity-30" aria-label="Next page"><ChevronRight size={17} /></button></div>}</div>
          {loading ? <div className="flex min-h-[360px] items-center justify-center text-slate-400"><div className="text-center"><div className="mx-auto mb-3 h-7 w-7 animate-spin rounded-full border-2 border-yellow-500/30 border-t-yellow-500" /><p>Loading staff records…</p></div></div> : visibleUsers.length === 0 ? <div className="flex min-h-[300px] items-center justify-center text-slate-500">No staff members found.</div> : (
            <div className="overflow-x-auto"><table className="w-full min-w-[1250px] text-left text-sm"><thead className="bg-[#151f35] text-xs uppercase tracking-wider text-slate-400"><tr><th className="px-5 py-4">ID</th><th className="px-5 py-4">Staff</th><th className="px-5 py-4">Contact</th><th className="px-5 py-4">Role / Status</th><th className="px-5 py-4">Personal Information</th><th className="px-5 py-4">Address</th><th className="px-5 py-4">Actions</th></tr></thead><tbody className="divide-y divide-white/5">{visibleUsers.map((u, localIndex) => { const globalIndex = pageStart + localIndex; const s = normalize(u.status || "active").toLowerCase(); return <tr key={u.id || u.uid || `${u.email}-${globalIndex}`} className="hover:bg-white/[0.025]">
...
function Avatar({ user }) { const name = normalize(user.name || user.email || "U"); const initials = name.split(/\s+/).filter(Boolean).slice(0, 2).map((x) => x[0]).join("").toUpperCase() || "U"; const src = normalize(user.profile_pic); return <div className="flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-full border border-yellow-500/50 bg-yellow-500/10 font-bold text-yellow-400">{src ? <img src={src} alt="" loading="lazy" decoding="async" className="h-full w-full object-cover" /> : initials}</div>; }
