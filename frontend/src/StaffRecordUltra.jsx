...
export default function StaffRecordUltra() {
  ...
  return <div className="min-h-screen bg-[#080d1d] px-4 py-7 text-white md:px-8"><div className="mx-auto max-w-[1700px]">
    ...
    <div className="overflow-hidden rounded-2xl border border-white/10 bg-[#0e1528] shadow-2xl">{loading ? <div className="flex min-h-[360px] items-center justify-center"><Loader2 size={30} className="animate-spin text-yellow-500"/></div> : filtered.length === 0 ? <div className="flex min-h-[300px] items-center justify-center text-slate-500">No users found.</div> : <div className="overflow-x-auto"><table className="w-full min-w-[1450px] text-left text-sm"><thead className="bg-[#151f35] text-xs uppercase tracking-wider text-slate-400"><tr><th className="px-5 py-4">User ID</th><th className="px-5 py-4">User</th><th className="px-5 py-4">Contact</th><th className="px-5 py-4">Role / Status</th><th className="px-5 py-4">Address</th><th className="px-5 py-4">CRUD / Security</th></tr></thead><tbody className="divide-y divide-white/5">{filtered.map((u) => { const s=normalize(u.status||"active").toLowerCase(); const busy=busyId===String(u.id); return <tr key={u.id} className="hover:bg-white/[0.025]"><td className="px-5 py-5"><div className="space-y-2"><span className="block w-fit rounded-lg bg-yellow-500/10 px-3 py-2 font-black text-yellow-400">#{u.id}</span><span className="block text-[10px] font-bold text-slate-500">{u.employeeID||"No employee ID"}</span></div></td><td className="px-5 py-5"><div className="flex items-center gap-3"><Avatar user={u}/><div><div className="font-bold">{u.name||"Unnamed User"}</div><div className="mt-1 text-xs text-slate-500">{u.email||"No email"}</div></div></div></td><td className="px-5 py-5"><div className="space-y-2 text-xs text-slate-300"><div className="flex items-center gap-2"><Mail size={14} className="text-slate-500"/>{u.email||"—"}</div><div className="flex items-center gap-2"><Phone size={14} className="text-slate-500"/>{u.phone||"—"}</div></div></td><td className="px-5 py-5"><div className="mb-2 inline-flex rounded-full bg-yellow-500/15 px-3 py-1 text-xs font-semibold text-yellow-300">{u.role||u.category||"staff"}</div><div><span className={`rounded-full px-3 py-1 text-xs font-semibold ${s==="active"?"bg-emerald-500/15 text-emerald-300":s==="blocked"?"bg-red-500/15 text-red-300":"bg-orange-500/15 text-orange-300"}`}>{statusLabel(s)}</span></div></td><td className="px-5 py-5 text-xs text-slate-300"><div className="flex max-w-[260px] items-start gap-2"><MapPin size={14} className="mt-0.5 shrink-0 text-slate-500"/><span>{u.address||"No address"}</span></div></td><td className="px-5 py-5"><div className="flex flex-wrap gap-2"><button onClick={() => navigate(`/user/${u.id}`)} className="inline-flex items-center gap-1.5 rounded-lg bg-blue-500/15 px-3 py-2 text-xs font-black text-blue-300"><Eye size={14}/>View</button><button onClick={() => openEdit(u)} disabled={busy} className="inline-flex items-center gap-1.5 rounded-lg bg-white/5 px-3 py-2 text-xs font-black"><Edit3 size={14}/>Edit</button>{s==="active"?<button onClick={() => setStatusForUser(u,"inactive")} disabled={busy} className="inline-flex items-center gap-1.5 rounded-lg bg-yellow-500/10 px-3 py-2 text-xs font-black text-yellow-300"><Ban size={14}/>Disable</button>:<button onClick={() => setStatusForUser(u,"active")} disabled={busy} className="inline-flex items-center gap-1.5 rounded-lg bg-emerald-500/10 px-3 py-2 text-xs font-black text-emerald-300"><CheckCircle2 size={14}/>Activate</button>}{s==="blocked"?<button onClick={() => setStatusForUser(u,"active")} disabled={busy} className="inline-flex items-center gap-1.5 rounded-lg bg-cyan-500/10 px-3 py-2 text-xs font-black text-cyan-300"><ShieldOff size={14}/>Unblock</button>:<button onClick={() => setStatusForUser(u,"blocked")} disabled={busy} className="inline-flex items-center gap-1.5 rounded-lg bg-red-500/10 px-3 py-2 text-xs font-black text-red-300"><LockKeyhole size={14}/>Block</button>}<button onClick={() => remove(u)} disabled={busy} className="inline-flex items-center gap-1.5 rounded-lg bg-red-500/10 px-3 py-2 text-xs font-black text-red-300"><Trash2 size={14}/>Delete</button>{busy&&<Loader2 size={17} className="animate-spin self-center text-yellow-400"/>}</div></td></tr>; })}</tbody></table></div>}</div>
  </div>
  {editing && <div className="fixed inset-0 z-[120] overflow-y-auto bg-black/75 p-4 backdrop-blur-sm"><form onSubmit={save} className="mx-auto mt-6 max-w-3xl rounded-3xl border border-white/10 bg-[#0b1220] p-5 shadow-2xl md:p-7"><div className="mb-6 flex items-center justify-between"><div className="flex items-center gap-4"><Avatar user={form} large/><div><h2 className="text-2xl font-black">Edit User</h2><p className="text-xs text-slate-500">Numeric ID: #{editing.id} • {editing.employeeID||"No employee ID"}</p></div></div><button type="button" onClick={() => setEditing(null)} className="rounded-xl border border-white/10 p-2"><X size={18}/></button></div><div className="grid gap-4 md:grid-cols-2"><Field label="Name" value={form.name} onChange={(v)=>updateForm("name",v)}/><Field label="Email (unique / read only)" value={form.email} onChange={()=>{}} disabled/><Field label="Phone" value={form.phone} onChange={(v)=>updateForm("phone",v)}/><Select label="Role" value={form.role} onChange={(v)=>updateForm("role",v)} options={["electrician","cro","admin","superadmin"]}/><Select label="Status" value={form.status} onChange={(v)=>updateForm("status",v)} options={["active","inactive","blocked"]}/><Select label="Marital status" value={form.maritalStatus} onChange={(v)=>updateForm("maritalStatus",v)} options={["Single","Married","Engaged","Divorced","Widowed","Prefer not to say"]}/><Field label="Address" value={form.address} onChange={(v)=>updateForm("address",v)}/><Field label="Background / Bio" value={form.backgroundInfo} onChange={(v)=>updateForm("backgroundInfo",v)}/><label className="md:col-span-2 block text-sm font-bold text-slate-300">Profile photo<input type="file" accept="image/jpeg,image/png,image/webp" onChange={(e)=>setPhoto(e.target.files?.[0]||null)} className="mt-2 block w-full rounded-xl border border-white/10 bg-white/5 p-3 text-xs"/></label></div><div className="mt-6 flex justify-end gap-2 border-t border-white/10 pt-5"><button type="button" onClick={()=>setEditing(null)} className="rounded-xl border border-white/10 px-4 py-2.5 text-sm font-black">Cancel</button><button disabled={busyId===String(editing.id)} className="inline-flex items-center gap-2 rounded-xl bg-yellow-500 px-5 py-2.5 text-sm font-black text-black disabled:opacity-60">{busyId===String(editing.id)&&<Loader2 size={16} className="animate-spin"/>}Save Changes</button></div></form></div>}
  </div>;
}
