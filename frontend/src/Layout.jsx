...
import { Bell, Bot, ChevronDown, ChevronRight, CircuitBoard, ClipboardList, Cpu, CalendarClock, Fuel, Home, LayoutDashboard, LogOut, Map, PanelsTopLeft, Plus, UserCircle, UserPlus, Users, Wrench, X, SlidersHorizontal } from "lucide-react";
...
const resolvePhotoUrl = (user) => {
  ...
  const origin = String(import.meta.env.VITE_API_ORIGIN || import.meta.env.VITE_API_URL || "").replace(/\/api\/?$/i, "").replace(/\/+$/g, "") || "http://localhost:5000";
    ...
    return (
      <div className="min-h-screen bg-[#0a0f1e] text-white overflow-x-hidden">
        <header className="fixed top-0 left-0 right-0 z-50 h-16 md:h-20 flex items-center gap-2 px-2 sm:px-4 md:px-6 bg-[#020617]/95 backdrop-blur-xl border-b border-white/5">
          <Link to="/" className="h-11 w-11 md:w-auto rounded-xl bg-yellow-500 text-black flex items-center justify-center md:justify-start gap-2 px-2 md:px-3 shrink-0"><Home size={20} /><span className="hidden md:inline text-[10px] font-black uppercase">Home</span></Link>
          <div className="hidden lg:block shrink-0"><h1 className="text-xl font-black italic">POWER<span className="text-yellow-500 not-italic">HOUSE</span></h1><p className="text-[8px] text-slate-600 uppercase tracking-[0.3em] font-black">Public Monitor</p></div>
          <nav className="flex-1 min-w-0 overflow-x-auto overscroll-x-contain"><div className="w-max min-w-max flex items-center gap-1 p-1 rounded-2xl bg-white/[0.03] border border-white/5">{publicLinks.map(([label, path, Icon]) => <Link key={path} to={path} className={`px-3 sm:px-4 md:px-5 py-2.5 rounded-xl text-[9px] sm:text-[10px] md:text-xs font-black uppercase whitespace-nowrap ${isActive(path) ? "bg-yellow-500 text-black" : "text-slate-400 hover:text-white"}`}><Icon size={14} className="inline mr-1" />{label}</Link>)}</div></nav>
